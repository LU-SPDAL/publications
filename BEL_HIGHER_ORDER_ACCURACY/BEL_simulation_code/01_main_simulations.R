# Main simulation study for the BEL paper.
#
# The AR(1) series is standardized to Var(Y_t) = 1. Coverage is based on
# 100,000 replications and interval length on the first 20,000 replications
# from the same streams. Feasible confidence intervals recompute the variance
# calibration at each candidate mean.
...
CODE_VERSION <- "master_bel_point_ci_v1_20260715"
...
# Methods used in the compact figures. The tables contain all nine methods.
CORE_METHODS <- c(
  "raw",
  "K_leading",
  "V_oracle",
...
# Comment out a module here if only part of the study is needed.
RUN_MODULES <- c(
  "block_power",
  "constant_sensitivity",
  "fixed_N_sweep",
  "regular_distribution_dependence",
  "extreme_distribution_dependence",
  "fixed_N_distribution_dependence"
)

RESULTS_DIR <- file.path(
  getwd(),
  paste0("MASTER_BEL_point_and_CI_results_", MODE)
)
...
contaminated_variance <- 0.95 * 1 + 0.05 * 25
contaminated_fourth <- 3 * (0.95 * 1^2 + 0.05 * 25^2)
contaminated_excess <- contaminated_fourth / contaminated_variance^2 - 3
...
innovation_info <- data.frame(
  ...
  assumption_status = c(
    "inside formal moment assumptions",
    "inside formal moment assumptions",
    "inside formal moment assumptions",
    "inside formal moment assumptions",
    "inside formal moment assumptions",
    "outside formal 8+delta moment assumption"
  ),
  severity_group = c(
    "regular",
    "regular",
...
regular_distributions <- innovation_info$innovation[
  innovation_info$severity_group == "regular"
]
extreme_distributions <- innovation_info$innovation[
  innovation_info$severity_group == "extreme"
]

# 2. Simulation designs
...
add_membership <- function(
    ...
    Q,
    x_order,
    alpha_nominal = NA_real_,
    C_nominal = NA_real_,
    note = "") {

  stopifnot(length(N) == length(M), length(M) == length(Q))
  stopifnot(all(N == M * Q))
...
# 2A. Five Gaussian power regimes, phi = 0.5

k <- c(4L, 5L, 6L, 8L, 10L, 16L, 20L)
membership_parts[[length(membership_parts) + 1L]] <- add_membership(
  module = "block_power",
  panel = "alpha_1_3",
  series = "alpha = 1/3",
  innovation = "gaussian",
...
k <- c(8L, 10L, 15L, 20L, 32L, 64L, 90L)
membership_parts[[length(membership_parts) + 1L]] <- add_membership(
  module = "block_power",
  panel = "alpha_1_2",
  series = "alpha = 1/2",
...
k <- c(4L, 5L, 6L, 7L, 8L, 10L, 16L, 20L, 27L)
membership_parts[[length(membership_parts) + 1L]] <- add_membership(
  module = "block_power",
  panel = "alpha_2_3",
  series = "alpha = 2/3",
  ...
  x_order = k^3,
  alpha_nominal = 2 / 3,
  note = "M = N^(2/3); smallest Q values are boundary designs"
)

# 2B. Constant sensitivity M = C sqrt(N), Gaussian phi = 0.5

for (N_now in c(64L, 100L, 400L, 1024L, 4096L, 8100L)) {
  rootN <- as.integer(round(sqrt(N_now)))
  stopifnot(rootN^2 == N_now)

  for (C_now in c(0.5, 1, 2)) {
    M_now <- as.integer(round(C_now * rootN))
    Q_now <- as.integer(N_now / M_now)
    stopifnot(M_now * Q_now == N_now)

    membership_parts[[length(membership_parts) + 1L]] <- add_membership(
      module = "constant_sensitivity",
      panel = paste0("N_", N_now),
      series = paste0("N = ", N_now),
      innovation = "gaussian",
      phi = 0.5,
      N = N_now,
      M = M_now,
...
# 2C. Fixed-N Gaussian block sweeps, phi = 0.5

for (fixed_spec in list(
  list(N = 400L, M = c(5L, 10L, 20L, 40L, 80L)),
  ...
  membership_parts[[length(membership_parts) + 1L]] <- add_membership(
    module = "fixed_N_sweep",
    panel = paste0("N_", N_now),
    series = paste0("N = ", N_now),
    innovation = "gaussian",
    phi = 0.5,
    N = rep(N_now, length(M_now)),
    M = M_now,
    Q = Q_now,
    x_order = M_now,
    note = "Direct fixed-N block-length comparison"
  )
}

# 2D. Regular distributions and all three dependence strengths

regular_N <- c(64L, 100L, 225L, 400L, 1024L, 4096L, 8100L)
for (dist_now in regular_distributions) {
  for (phi_now in c(0.2, 0.5, 0.8)) {
    rootN <- as.integer(round(sqrt(regular_N)))
    stopifnot(all(rootN^2 == regular_N))

    membership_parts[[length(membership_parts) + 1L]] <- add_membership(
      module = "regular_distribution_dependence",
      panel = paste0(dist_now, "_phi_", phi_now),
      series = paste0(dist_now, ", phi = ", phi_now),
      innovation = dist_now,
      phi = phi_now,
      N = regular_N,
      M = rootN,
      ...
      x_order = regular_N,
      alpha_nominal = 1 / 2,
      note = "Balanced distribution/dependence design"
    )
  }
}

# 2E. Extreme distributions, phi in {0.5,0.8}

extreme_N <- c(64L, 100L, 225L, 400L, 1024L, 4096L)
for (dist_now in extreme_distributions) {
  for (phi_now in c(0.5, 0.8)) {
    rootN <- as.integer(round(sqrt(extreme_N)))
    stopifnot(all(rootN^2 == extreme_N))

    membership_parts[[length(membership_parts) + 1L]] <- add_membership(
      module = "extreme_distribution_dependence",
      panel = paste0(dist_now, "_phi_", phi_now),
      series = paste0(dist_now, ", phi = ", phi_now),
      innovation = dist_now,
      phi = phi_now,
      ...
      x_order = extreme_N,
      alpha_nominal = 1 / 2,
      note = "Balanced extreme-distribution design"
    )
  }
}

# 2F. Fixed N=4096, M in {16,64,256}, all assigned distributions and phi

for (dist_now in innovation_info$innovation) {
  phi_set <- if (dist_now %in% regular_distributions) {
    c(0.2, 0.5, 0.8)
  } else {
    c(0.5, 0.8)
  }

  for (phi_now in phi_set) {
    M_now <- c(16L, 64L, 256L)
    membership_parts[[length(membership_parts) + 1L]] <- add_membership(
      module = "fixed_N_distribution_dependence",
      panel = paste0(dist_now, "_phi_", phi_now),
      series = paste0(dist_now, ", phi = ", phi_now),
      innovation = dist_now,
      phi = phi_now,
      N = rep(4096L, length(M_now)),
      M = M_now,
      Q = as.integer(4096L / M_now),
      x_order = M_now,
      note = "Fixed N=4096 distribution/dependence block comparison"
    )
  }
}
...
membership$phi_label <- paste0("phi = ", membership$phi)
...
# Unique distribution-phi-N-M designs are simulated only once even if they
# occur in several modules.
...
# A scenario is a distribution-phi-N combination. All required block lengths
# for that generated series are evaluated on the same Monte Carlo sample.
...
write.csv(
  results_unique,
  file.path(RESULTS_DIR, "tables", "master_results_unique_designs.csv"),
  row.names = FALSE
)
write.csv(
  results_by_module,
  file.path(RESULTS_DIR, "tables", "master_results_by_module.csv"),
  row.names = FALSE
)
write.csv(
  diagnostics_unique,
  file.path(RESULTS_DIR, "tables", "diagnostics_unique_designs.csv"),
  row.names = FALSE
)
write.csv(
...
results_by_module$statistic <- factor(
  results_by_module$statistic,
  levels = method_names
)
results_by_module$stat_label <- factor(
  method_labels[as.character(results_by_module$statistic)],
...
plot_core <- function(data) {
  data[
    as.character(data$statistic) %in% CORE_METHODS,
  ]
}

# 8A. Five power regimes: coverage and mean length

if ("block_power" %in% RUN_MODULES) {
  dd <- plot_core(
    results_by_module[results_by_module$module == "block_power", ]
  )
  dd$series <- factor(
    dd$series,
    levels = c(
      "alpha = 1/3", "alpha = 2/5", "alpha = 1/2",
      "alpha = 3/5", "alpha = 2/3"
    )
  )
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Five block-length power regimes: coverage",
      x = "Sample size N",
  ...
  save_plot(p, "block_power_coverage_core.png", 12, 7)
  ...
  p_len <- ggplot2::ggplot(
    ...
    ggplot2::facet_wrap(~ series, scales = "free_x") +
    ggplot2::labs(
      title = "Five block-length power regimes: normalized mean interval length",
      x = "Sample size N",
  ...
  save_plot(p_len, "block_power_length_core.png", 12, 7)
}

# 8B. Constant sensitivity

if ("constant_sensitivity" %in% RUN_MODULES) {
  dd <- plot_core(
    results_by_module[
      results_by_module$module == "constant_sensitivity",
    ]
  )
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = "Constant sensitivity: coverage",
      x = expression(C ~ "in" ~ M == C * N^{1/2}),
      y = "Empirical coverage",
      linetype = NULL,
      shape = NULL
    ) +
  ...
  save_plot(p, "constant_sensitivity_coverage_core.png", 13, 8)
  ...
  p_len <- ggplot2::ggplot(
    ...
    ggplot2::facet_wrap(~ series) +
    ggplot2::labs(
      title = "Constant sensitivity: normalized mean interval length",
      x = expression(C ~ "in" ~ M == C * N^{1/2}),
      y = "Mean length / first-order benchmark",
      linetype = NULL,
      shape = NULL
  ...
  save_plot(p_len, "constant_sensitivity_length_core.png", 13, 8)
}

# 8C. Fixed-N Gaussian block sweeps

if ("fixed_N_sweep" %in% RUN_MODULES) {
  dd <- plot_core(
    results_by_module[results_by_module$module == "fixed_N_sweep", ]
  )
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::facet_wrap(~ series, scales = "free_x") +
    ggplot2::labs(
      title = "Fixed-N block-length sweeps: coverage",
      x = "Block length M (log2 scale)",
      y = "Empirical coverage",
      linetype = NULL,
      shape = NULL
    ) +
  ...
  save_plot(p, "fixed_N_sweep_coverage_core.png", 11, 6)
  ...
  p_len <- ggplot2::ggplot(
    ...
    ggplot2::facet_wrap(~ series, scales = "free_x") +
    ggplot2::labs(
      title = "Fixed-N block-length sweeps: normalized mean interval length",
      x = "Block length M (log2 scale)",
      y = "Mean length / first-order benchmark",
      linetype = NULL,
      shape = NULL
  ...
  save_plot(p_len, "fixed_N_sweep_length_core.png", 11, 6)
}

# 8D. Regular distributions and dependence strengths

if ("regular_distribution_dependence" %in% RUN_MODULES) {
  dd <- plot_core(
    results_by_module[
      results_by_module$module == "regular_distribution_dependence",
    ]
  )
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::facet_grid(phi_label_module ~ innovation_label_module) +
    ggplot2::labs(
      title = "Regular distributions and dependence strengths: coverage",
      subtitle = "Balanced blocks M = Q = sqrt(N)",
      x = "Sample size N",
      y = "Empirical coverage",
      linetype = NULL,
      shape = NULL
  ...
  save_plot(p, "regular_distribution_dependence_coverage_core.png", 14, 10)
  ...
  p_len <- ggplot2::ggplot(
    ...
    ggplot2::facet_grid(phi_label_module ~ innovation_label_module) +
    ggplot2::labs(
      title = "Regular distributions: normalized mean interval length",
      subtitle = "Balanced blocks M = Q = sqrt(N)",
      x = "Sample size N",
      y = "Mean length / first-order benchmark",
      linetype = NULL,
  ...
  save_plot(p_len, "regular_distribution_dependence_length_core.png", 14, 10)
}

# 8E. Extreme distributions

if ("extreme_distribution_dependence" %in% RUN_MODULES) {
  dd <- plot_core(
    results_by_module[
      results_by_module$module == "extreme_distribution_dependence",
    ]
  )
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::facet_grid(phi_label_module ~ innovation_label_module) +
    ggplot2::labs(
      title = "Extreme innovation distributions: coverage",
      subtitle = "Balanced blocks; t(5) is outside the formal high-moment assumption",
      x = "Sample size N",
      y = "Empirical coverage",
      linetype = NULL,
      shape = NULL
  ...
  save_plot(p, "extreme_distribution_coverage_core.png", 14, 8)
  ...
  p_len <- ggplot2::ggplot(
    ...
    ggplot2::facet_grid(phi_label_module ~ innovation_label_module) +
    ggplot2::labs(
      title = "Extreme distributions: normalized mean interval length",
      subtitle = "Balanced blocks; t(5) is outside the formal high-moment assumption",
      x = "Sample size N",
      y = "Mean length / first-order benchmark",
      linetype = NULL,
  ...
  save_plot(p_len, "extreme_distribution_length_core.png", 14, 8)
}

# 8F. Fixed N=4096 distribution/dependence block comparison

if ("fixed_N_distribution_dependence" %in% RUN_MODULES) {
  dd <- plot_core(
    results_by_module[
      results_by_module$module == "fixed_N_distribution_dependence",
    ]
  )
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::facet_grid(phi_label_module ~ innovation_label_module) +
    ggplot2::labs(
      title = "Fixed N=4096 block comparison across distributions",
      x = "Block length M (log2 scale)",
      y = "Empirical coverage",
      linetype = NULL,
      shape = NULL
  ...
  save_plot(p, "fixed_N4096_distribution_coverage_core.png", 16, 12)
  ...
  p_len <- ggplot2::ggplot(
    ...
    ggplot2::facet_grid(phi_label_module ~ innovation_label_module) +
    ggplot2::labs(
      title = "Fixed N=4096: normalized mean interval length",
      x = "Block length M (log2 scale)",
      y = "Mean length / first-order benchmark",
      linetype = NULL,
      shape = NULL
  ...
  save_plot(p_len, "fixed_N4096_distribution_length_core.png", 16, 12)
}
...
manifest <- c(
  ...
  paste("Stationary tail variance tolerance:", STATIONARY_TAIL_VARIANCE_TOL),
  paste("Number of module-membership rows:", nrow(membership)),
  paste("Number of unique designs:", nrow(design_table)),
  paste("Number of generated-data scenarios:", nrow(scenario_table)),
  paste("Modules:", paste(RUN_MODULES, collapse = ", ")),
  paste(
    "Bandwidth:",
  ...
  paste(
    "Full correction:",
    "common explicit asymptotic order-one Kitamura factor"
  ),
  paste(
    "Results directory:",
...
readme <- c(
  "MASTER BEL SIMULATION OUTPUT",
  "",
  "Primary tables:",
  "  master_results_unique_designs.csv",
  "  master_results_by_module.csv",
  "  diagnostics_unique_designs.csv",
  ...
  "The total confidence-set length is reported; the component containing",
  "the sample mean is also summarized. Disconnected and nonregular rates",
  "are recorded separately.",
  "",
  "The standardized t(5) case is outside the formal 8+delta moment",
  "assumption and must be described as an outside-theory robustness case."
)
