...
# dati <- rnorm(100)
  for(i in 1:N){
  dati<-izlase_norm2(n,0.9,0,0,1,25)
...
#STMemp
  ...
  for(i in 1:4){
    ...
    cov.acc<-cov.acc.tab%>%
      ...
      geom_point(size=2,aes(lty=method.gr2,color=method.gr1,shape=method.gr2))+
      geom_line(size=1,aes(lty=method.gr2,color=method.gr1))+
      scale_color_manual(values=c(viridis_pal()(4),"purple"),labels=c(
      lapply(sprintf('$STM\\gamma=%#.1f$',gamma.val),TeX),"TM"))+
      geom_hline(yintercept=0.95,lty="dashed",color="black")+
      ggtitle(paste("N=",j[i]))+
      xlab(TeX('$\\alpha$'))+
      ylab('coverage accuraccy')+
      theme_minimal()+
      theme(plot.title=element_text(hjust=0.5))+
      labs(shape="Method",lty="Method",color="Estimator")
    ...
    int.len<-int.len.tab%>%
      ...
      ggtitle(paste("N=",j[i]))+
      xlab(TeX('$\\alpha$'))+
      ylab('average length of CI')+
      theme_minimal()+
      theme(plot.title=element_text(hjust=0.5))+
      labs(shape="Method",linetype="Method",color="Estimator")
...
for(n in c(50,100,200,500)){
  ...
  for(i in 1:N){
    # dati<-izlase_norm2(n,0.9,0,0,1,25)
    dati <- izlase_norm3(n, 0.1,0.8,0.1, -10,0,10,1,1,1)
    # hist(dati)
    for(a in alpha.val){
      Tmu<-mean(dati,trim=a)
      var.tmean<-trimvar(dati,alpha=a)
